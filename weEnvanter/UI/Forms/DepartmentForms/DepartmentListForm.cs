    ...
    public partial class DepartmentListForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void btn_ExportPDF_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                _systemLogService.LogActivity(
                    ...
                    "Departman Listesi PDF Dışa Aktarıldı",
                    $"{userFullName} {now} tarihinde departman listesini PDF olarak dışa aktardı.",
                    "Department",
                    null,
                    weEnvanter.Domain.Enums.LogType.Information.ToString()
                );
                Core.Helpers.ExportHelper.ExportToPdf(gridControl_Departments, "weEnvanter_DepartmanListesi");
            }
            catch (Exception ex)
            {
        ...
        private void btn_ExportXLSX_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                _systemLogService.LogActivity(
                    Program.CurrentUser?.Id,
                    "Departman Listesi Excel Dışa Aktarıldı",
                    $"{userFullName} {now} tarihinde departman listesini Excel olarak dışa aktardı.",
                    "Department",
                    null,
                    weEnvanter.Domain.Enums.LogType.Information.ToString()
                );
                Core.Helpers.ExportHelper.ExportToExcel(gridControl_Departments, "weEnvanter_DepartmanListesi");
            }
            catch (Exception ex)
            {
                ...
                try
                {
                        ...
                        MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        _systemLogService.LogActivity(
                            Program.CurrentUser?.Id,
                            "Departman Silindi",
                            $"{userFullName} {now} tarihinde '{selectedRow.Name}' isimli departmanı sildi.",
                            "Department",
